<div id="datakesehatan-part" class="content" role="tabpanel" aria-labelledby="information-part-trigger">
  <div class="form-section">
            ...
            @foreach (['tengkurap', 'duduk', 'merangkak', 'berdiri', 'berjalan'] as $field)
              <div class="col-12 col-sm-6 col-md-2">
                ...
                <input name="{{ $field }}" class="form-control" id="{{ $field }}" type="text" placeholder="bulan" required
                  value="{{ $data['riwsehatperkembangan']->$field ?? '' }}" pattern="^-?\d*$" title="Harap masukkan data yang sesuai" />
              </div>
            @endforeach
          </div>
        </div>
    ...
    <div class="mt-4 mb-3">
      ...
      <textarea class="form-control" id="catatanTambahan" name="catatan_tambahan_form4" rows="3" required>
        {{ $data['riwsehatperkembangan']->catatan_tambahan ?? '' }}
      </textarea>
    </div>
